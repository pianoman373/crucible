...
vec3 lighting(vec3 fragPos, vec3 albedo, vec3 normal, float roughness, float metallic) {
	...
	float shadow0 = mix(1.0, ShadowCalculation(0.0001, fragLightSpace0, shadowTextures[0]), inShadow(fragLightSpace0));
    float shadow1 = mix(1.0, ShadowCalculation(0.0002, fragLightSpace1, shadowTextures[1]), inShadow(fragLightSpace1) * (1-inShadow(fragLightSpace0)));
    float shadow2 = mix(1.0, ShadowCalculation(0.0003, fragLightSpace2, shadowTextures[2]), inShadow(fragLightSpace2) * (1-inShadow(fragLightSpace1)));
    float shadow3 = mix(1.0, ShadowCalculation(0.0004, fragLightSpace3, shadowTextures[3]), inShadow(fragLightSpace3) * (1-inShadow(fragLightSpace2)));
